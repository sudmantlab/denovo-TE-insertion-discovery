rule genomicCoverage:
    ...
    shell:
        ...
        samtools coverage {input.bam} > {output.genomicCoverageTab} | awk 'NR>1 {{len=$$3-$$2+1; sum += len*$$7; total += len}} END {{print "Mean genome coverage =", sum/total}}' > {output.genomicCoverageTab}
        """
